...
export default function CadastroGestante() {
  ...
  useEffect(() => {
    console.log("📊 Debug doulas:", { doulas, loading, error });
  }, [doulas, loading, error]);
  ...
  if (loading) {
    return (
      <div className="h-screen flex items-center justify-center text-purple-600 text-lg">
        Carregando doulas...
      </div>
    );
  }
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-4 px-4">
        <p className="text-gray-600 text-lg text-center">
          Nenhuma doula disponível no momento.
        </p>
        <Link 
          to="/" 
          className="text-purple-600 hover:underline"
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-100 to-purple-300 px-4 py-10">
      <div className="bg-white shadow-2xl rounded-2xl p-8 w-full max-w-md text-center space-y-6">
        ...
        <div>
          {doula.foto ? (
            <img 
              src={doula.foto} 
              alt={doula.name} 
              className="w-32 h-32 rounded-full mx-auto border-4 border-purple-300 object-cover shadow" 
            />
          ) : (
        ...
        <Link 
          to="/" 
          className="text-sm text-purple-600 hover:underline transition block mt-4"
        >
          ← Voltar para a escolha de perfil
        </Link>
      </div>
